...
import { ArrowLeft, Swords, Copy, Check, Info, X } from "lucide-react";

/**
 * Game server for online multiplayer.
 * - Local dev: run `npm run server` (server.js at the repo root, port 8377).
 * - Override for production by setting VITE_AP_SERVER (e.g. wss://games.yourdomain.com).
 */
const DEFAULT_SERVER = (import.meta.env.VITE_AP_SERVER as string | undefined) || "ws://localhost:8377";
...
export default function AaduPuliAatamGamePage() {
  ...
  const gameSrc = `/aadupuliatam/index.html?${qs.toString()}`;
  ...
  return (
    <div className="h-dvh w-full flex flex-col bg-[#0C2419]">
      {/* Slim control bar */}
      <div className="flex items-center justify-between gap-3 px-3 sm:px-5 py-2.5 bg-[#0C2419] border-b border-[#1F3D5E] text-white shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="hidden sm:flex items-center gap-1.5 text-sm font-black tracking-wide min-w-0">
            <Swords size={15} className="text-[#F6B828] shrink-0" />
            <span className="truncate">Aadu Puli Aatam</span>
          </span>
        </div>
          ...
          {serverReady !== null && (
            <span
              ...
              title={serverReady
                ...
                : `No game server at ${server} — online mode will be offline. Start it with "npm run server" or set VITE_AP_SERVER.`}
            >
      ...
      <div className="flex-1 min-h-0 relative bg-[#FCFAF5]">
        <iframe
          src={gameSrc}
          title="Aadu Puli Aatam — Goats &amp; Tigers, ancient Tamil Nadu board game"
          className="absolute inset-0 w-full h-full border-0"
          allow="clipboard-write; autoplay"
        />
        {serverReady === false && !bannerDismissed && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10 max-w-md w-[calc(100%-24px)] bg-[#0C2419]/95 backdrop-blur text-white text-[11px] font-medium px-4 py-2.5 rounded-xl border border-[#1F3D5E] shadow-2xl flex items-start gap-2">
            ...
            <span className="flex-grow">
              ...
              <code className="text-[#F6B828]">npm run server</code> and set <code className="text-[#F6B828]">VITE_AP_SERVER</code>.
            </span>
            <button
